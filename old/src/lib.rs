...
use crate::{config, fio, utils};

pub fn parse_md(filename: PathBuf, output_type: &str, specific_type: Option<&str>) {
    ...
    let html_filename = format!("{}.html", filename.to_str().unwrap().split(".").next().unwrap());
    ...
    match output_type {
        ...
        "pdf" => {
            ...
            match tab.print_to_pdf(Some(options)) {
                Ok(pdf_data) => {
                    std::fs::write(
                        format!("{}.pdf", filename.to_str().unwrap().split(".").next().unwrap()),
                        pdf_data,
                    )
                    .expect("Failed writing Pdf");
                }
                Err(err) => {
            ...
            match specific_type {
                Some("png") => {
                    ...
                    match tab.capture_screenshot(
                        ...
                        Ok(png_data) => {
                            std::fs::write(
                                format!("{}.png", filename.to_str().unwrap().split(".").next().unwrap()),
                                png_data,
                            )
                ...
                Some("jpeg") => {
                    ...
                    match tab.capture_screenshot(
                        ...
                        Ok(jpeg_data) => {
                            std::fs::write(
                                format!("{}.jpeg", filename.to_str().unwrap().split(".").next().unwrap()),
                                jpeg_data,
                            )
                            .expect("Failed drawing Image");
                        }
