...
use std::process;
use utils::{BLUE_CODE, MAGENTA_CODE, NORMAL_CODE, RED_CODE, VERSION};
    ...
    match args.len() {
        2 => {
            match args.get(1).unwrap().as_str() {
                "--help" => {
                    println!(
                        "{}Usage:{} {}smd{} {}[input_file] [output_type] [*specific_type]{}",
                        RED_CODE, NORMAL_CODE, BLUE_CODE, NORMAL_CODE, MAGENTA_CODE, NORMAL_CODE
                    );
                    ...
                    println!(
                        "  {}--init{}\t\t\tInitialize smd. Needed for the first use.",
                        MAGENTA_CODE, NORMAL_CODE
                    );
                    println!(
                        "  {}--flavour{}\t\t\tSet flavour for the md files.",
                        MAGENTA_CODE, NORMAL_CODE
                    );
                    println!(
                    ...
                    println!(
                        "  {}--output [file]{}\t\tSpecify output type",
                        MAGENTA_CODE, NORMAL_CODE
                    );
                    println!(
                    ...
                    println!(
                        "  {}--specific [type]{}\t\tSpecify specific output type",
                        MAGENTA_CODE, NORMAL_CODE
                    );
                }
                "--version" => {
                ...
                "--init" => {
                    ...
                    if config::get_path_to_config_file().exists() {
                        println!(
                            "{}smd{} already initialized! Use {}smd --help{} for more information.",
                            BLUE_CODE, NORMAL_CODE, BLUE_CODE, NORMAL_CODE
                        );
                        process::exit(1);
                    }
            ...
            match args.get(1).unwrap().as_str() {
                "--input" => {
                    if args.get(3).unwrap().as_str() == "--output" {
                        parse_md(
                            PathBuf::from(args.get(2).unwrap().as_str()),
                ...
                "--output" => {
                    if args.get(3).unwrap().as_str() == "--input" {
                        parse_md(
                            PathBuf::from(args.get(4).unwrap().as_str()),
...
fn handle_init_not_called() {
    if !config::get_path_to_config_file().exists() {
        println!(
            "{}smd{} is not initialized! Use {}smd --init{} to initialize",
            BLUE_CODE, NORMAL_CODE, BLUE_CODE, NORMAL_CODE
        );
        process::exit(1);
    }
