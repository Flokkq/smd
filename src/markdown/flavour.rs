...
impl Flavour {
    ...
    pub fn set_flavour(settings: Settings) {
        write_file(settings.config_folder.join("current-flavour.css"), settings.flavour.to_string());
    }

    pub fn get_current_flavour(settings: Settings) -> Flavour  {
        let mut flavour = settings.flavour;
    ...
    pub fn add_flavour(settings: Settings, flavour: Flavour, path: PathBuf) {
        let mut flavours = Flavour::load_flavours(&settings);
